...
public class StreamsExample {
    ...
    public static void main(final String[] args) {
        ...
        // TODO With functional interfaces declared
           ...
           Consumer<Author> printAuthor = new Consumer<Author>() {
            @Override
            public void accept(Author author) {
                System.out.println(author);
            }
        ...
        // TODO With functional interfaces declared
         ...
         Function<Author, List<Book>> getBooksFunction = new Function<Author, List<Book>>() {
            @Override
            public List<Book> apply(Author author) {
                return author.books;
            }
        };
        Consumer<Book> printBook = new Consumer<Book>() {
            @Override
        ...
        // TODO With functional interfaces declared
         Function<Author, List<Book>> getAuthorBooks = new Function<Author, List<Book>>() {
            @Override
            public List<Book> apply(Author author) {
                return author.books;
            }
        };
        ToIntFunction<Book> getBookPrice = new ToIntFunction<Book>() {
            @Override
            public int applyAsInt(Book book) {
                return book.price;
            }
        };
        double averagePrice = authors
            .stream()
            .map(getAuthorBooks)
            .flatMap(books -> books.stream())
            .mapToInt(getBookPrice)
            .average()
            .orElse(0.0);
